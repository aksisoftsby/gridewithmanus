...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-6">
        <a href="{{ route('home') }}" class="inline-flex items-center text-purple-800 hover:underline font-medium text-sm">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            @if($item->category_name)
                <span class="text-[11px] font-bold uppercase bg-purple-100 text-purple-800 px-3 py-1 rounded-full">{{ $item->category_name }}</span>
            @endif
        ...
        @if($item->excerpt)
            <p class="text-base text-purple-800 font-medium mb-6">{{ $item->excerpt }}</p>
        @endif
