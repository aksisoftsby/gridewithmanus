...
@section('content')
<div class="bg-gradient-to-r from-emerald-600 to-teal-700 text-white py-12 mb-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-3xl font-extrabold tracking-tight mb-2">Iklan Gratis</h1>
        <p class="text-emerald-100 max-w-2xl mx-auto">Jual beli & jasa dalam satu ekosistem. Pasang iklan gratis langsung dari aplikasi Gride Customer.</p>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
    ...
    <form action="{{ route('iklan.index') }}" method="GET" class="bg-white p-3 rounded-2xl shadow-md flex flex-col sm:flex-row gap-3 mb-8">
        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari judul, deskripsi, atau kota..." class="flex-grow px-4 py-3 rounded-xl text-gray-800 border focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <a href="{{ route('iklan.detail', $item->id) }}" class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden hover:shadow-lg transition group">
                <div class="h-44 bg-gray-100 flex items-center justify-center overflow-hidden relative">
                    ...
                    @if($item->category_name)
                        <span class="absolute top-3 left-3 bg-emerald-600 text-white text-xs font-semibold px-3 py-1 rounded-full">{{ $item->category_name }}</span>
                    @endif
                </div>
                <div class="p-5">
                    <h3 class="font-bold text-gray-900 group-hover:text-emerald-600 transition line-clamp-2">{{ $item->title }}</h3>
                    ...
                    @if((float)$item->price > 0)
                        <div class="mt-3 font-bold text-emerald-600">Rp {{ number_format((float)$item->price, 0, ',', '.') }}</div>
                    @endif
                </div>
            </a>
        @empty
