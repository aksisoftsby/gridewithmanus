...
<div class="brand-gradient text-white py-12 mb-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="text-pink-100 max-w-2xl mx-auto">Jual beli & jasa dalam satu ekosistem. Pasang iklan gratis langsung dari aplikasi RideSip Customer.</p>
        <div class="mt-6 flex justify-center gap-3">
            <a href="https://ridesip.my.id/apk/customer.apk" class="bg-blue-400 text-gray-900 hover:bg-blue-300 px-5 py-2.5 rounded-full font-semibold shadow transition text-sm">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
    ...
    <form action="{{ route('iklan.index') }}" method="GET" class="bg-white p-3 rounded-2xl shadow-md flex flex-col sm:flex-row gap-3 mb-8">
        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari judul, deskripsi, atau kota..." class="flex-grow px-4 py-3 rounded-xl text-gray-800 border focus:outline-none focus:ring-2 focus:ring-pink-600">
            ...
            <a href="{{ route('iklan.detail', $item->id) }}" class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden hover:shadow-lg transition group">
                <div class="h-44 bg-gray-100 flex items-center justify-center overflow-hidden relative">
                    ...
                    @if($item->category_name)
                        <span class="absolute top-3 left-3 bg-pink-700 text-white text-xs font-semibold px-3 py-1 rounded-full">{{ $item->category_name }}</span>
                    @endif
                </div>
                <div class="p-5">
                    <h3 class="font-bold text-gray-900 group-hover:text-pink-700 transition line-clamp-2">{{ $item->title }}</h3>
                    ...
                    @if((float)$item->price > 0)
                        <div class="mt-3 font-bold text-pink-700">Rp {{ number_format((float)$item->price, 0, ',', '.') }}</div>
                    @endif
                </div>
            </a>
        @empty
