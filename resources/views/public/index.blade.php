...
<div class="brand-gradient text-white py-16 mb-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="text-lg text-purple-100 max-w-2xl mx-auto mb-8">Pesan makanan favorit, belanja kebutuhan harian, dan layanan kurir instan dalam satu aplikasi.</p>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
    ...
    <div class="mb-10">
        <h2 class="text-xl font-bold text-gray-800 mb-4 flex items-center">
            <i class="fa-solid fa-tags text-purple-700 mr-2"></i> Promo & Voucher Spesial
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($promos as $promo)
                <div class="bg-gradient-to-r from-purple-700 to-amber-600 text-white p-6 rounded-2xl shadow-md flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-xs text-purple-100 mt-1">Min. belanja Rp {{ number_format($promo->min_purchase, 0, ',', '.') }}</p>
                    </div>
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800 flex items-center">
            <i class="fa-solid fa-store text-purple-700 mr-2"></i> Mitra Merchant & Restoran Terdekat
        </h2>
        ...
        @forelse($merchants as $merchant)
            <a href="{{ route('merchant.detail', $merchant->slug) }}" class="bg-white rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden border border-gray-100 flex flex-col group">
                <div class="relative h-48 bg-gray-200 overflow-hidden">
                    ...
                    <span class="absolute top-3 left-3 bg-purple-700 text-white text-xs font-bold px-3 py-1 rounded-full shadow">
                        {{ $merchant->type }}
                    </span>
                ...
                <div class="p-6 flex flex-col flex-grow">
                    <h3 class="text-lg font-bold text-gray-900 group-hover:text-purple-700 transition">{{ $merchant->name }}</h3>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500 space-y-1">
                        <div class="flex justify-between items-center">
                            <span><i class="fa-solid fa-location-dot mr-1 text-purple-700"></i>{{ $merchant->address_line ?? '' }}{{ $merchant->city ? ', ' . $merchant->city : '' }}</span>
                            <span class="text-purple-700 font-semibold">Lihat Menu &rarr;</span>
                        </div>
                        @if($merchant->latitude && $merchant->longitude)
                            <a href="https://www.google.com/maps/dir/?api=1&destination={{ $merchant->latitude }},{{ $merchant->longitude }}" target="_blank" class="inline-flex items-center text-purple-800 hover:underline font-medium">
...
<div class="bg-white py-12 border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
            <i class="fa-solid fa-newspaper text-purple-700 mr-2"></i> Berita & Informasi Terbaru
        </h2>
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-2">
                            @if($item->category_name)
                                <span class="text-[10px] font-bold uppercase bg-purple-100 text-purple-800 px-2 py-0.5 rounded-full">{{ $item->category_name }}</span>
                            @endif
...
<div class="bg-gradient-to-br from-purple-50 to-amber-50 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
            <i class="fa-solid fa-quote-left text-purple-700 mr-2"></i> Kata Pelanggan Kami
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($testimonials as $t)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        @else
                            <div class="w-11 h-11 rounded-full bg-purple-100 flex items-center justify-center text-purple-800 font-bold">{{ strtoupper(substr($t->name, 0, 1)) }}</div>
                        @endif
                        <div>
