...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <a href="{{ route('iklan.index') }}" class="text-emerald-600 hover:underline text-sm font-semibold">&larr; Kembali ke Iklan Gratis</a>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    @if(count($photos) > 1)
                        <div class="flex gap-2 p-3 overflow-x-auto bg-gray-50">
                            @foreach($photos as $i => $photo)
                                <img src="{{ $photo }}" alt="Foto {{ $i + 1 }}" onclick="document.getElementById('mainPhoto').src='{{ $photo }}'" class="w-20 h-20 object-cover rounded-lg border-2 cursor-pointer hover:border-emerald-500 transition" onerror="this.src='/images/merchant-placeholder.svg';">
                            @endforeach
                        </div>
                    @endif
                @else
        ...
        <div class="bg-white rounded-2xl shadow border border-gray-100 p-6 h-fit">
            @if($item->category_name)
                <span class="bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1 rounded-full uppercase">{{ $item->category_name }}</span>
            @endif
            <h1 class="text-2xl font-extrabold text-gray-900 mt-3">{{ $item->title }}</h1>
            @if((float)$item->price > 0)
                <div class="text-2xl font-bold text-emerald-600 mt-2">Rp {{ number_format((float)$item->price, 0, ',', '.') }}</div>
            @endif
            ...
            <div class="mt-5 space-y-2 text-sm border-t pt-4">
                ...
                @if($item->contact_phone)
                    <div><span class="font-semibold text-gray-700">No. HP:</span>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $item->contact_phone) }}" target="_blank" class="text-emerald-600 hover:underline font-semibold">{{ $item->contact_phone }}</a>
                    </div>
                @endif
            </div>
            <a href="https://gride.web.id/apk/customer.apk" class="mt-6 block text-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 rounded-xl shadow transition text-sm">
    ...
    @if($related->count())
        <div class="mt-10">
            <h2 class="text-xl font-bold text-gray-900 mb-4"><i class="fa-solid fa-bullhorn text-emerald-600 mr-2"></i>Iklan Serupa</h2>
                    ...
                    <a href="{{ route('iklan.detail', $rel->id) }}" class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden hover:shadow-lg transition">
                        ...
                        <div class="p-3">
                            ...
                            @if((float)$rel->price > 0)
                                <div class="text-xs font-bold text-emerald-600 mt-1">Rp {{ number_format((float)$rel->price, 0, ',', '.') }}</div>
                            @endif
                        </div>
                    </a>
                @endforeach
