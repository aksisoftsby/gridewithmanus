...
<head>
    ...
    <title>{{ $title ?? 'Iklan Baris RideSip' }}</title>
...
<body class="bg-gray-100 text-gray-900 font-sans antialiased">
    ...
    <div class="brand-gradient text-white">
        <div class="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
            <div class="flex items-center space-x-2">
                ...
                <span class="font-bold tracking-wider">RideSip Iklan Baris</span>
            </div>
            <span class="text-xs text-white/80">ridesip.my.id</span>
        </div>
    </div>

    <main class="max-w-3xl mx-auto px-4 py-4">
        @if(session('success'))
            <div class="bg-pink-100 border-l-4 border-pink-600 text-pink-800 p-3 rounded shadow mb-4 text-sm" role="alert">
                <p>{{ session('success') }}</p>
            </div>
        @endif
        @if(session('error'))
    ...
    <div class="fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 shadow-lg z-50">
        <div class="max-w-3xl mx-auto grid grid-cols-3 text-center text-xs">
            <a href="{{ route('iklanwebview.index') }}" class="py-3 text-pink-700 hover:bg-pink-50 {{ request()->routeIs('iklanwebview.index') ? 'font-bold' : '' }}">
                <i class="fa-solid fa-list block text-lg mb-0.5"></i>Listing
            </a>
            <a href="{{ route('iklanwebview.my') }}" class="py-3 text-pink-700 hover:bg-pink-50 {{ request()->routeIs('iklanwebview.my') ? 'font-bold' : '' }}">
                <i class="fa-solid fa-user block text-lg mb-0.5"></i>Iklan Saya
            </a>
            <a href="{{ route('iklanwebview.create') }}" class="py-3 bg-pink-700 text-white hover:bg-pink-800 font-bold">
