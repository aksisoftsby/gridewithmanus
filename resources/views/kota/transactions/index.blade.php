...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Transactions — Transaksi Order</h1>
            <p class="text-sm text-gray-500">Rekap finansial order di area coverage Anda (view-only)</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-xl shadow border border-gray-100 mb-6">
        <form action="{{ route('kota.transactions.index') }}" method="GET" class="flex flex-wrap gap-3 items-end">
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Cari</label>
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Order number / status pembayaran" class="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none w-72">
            </div>
            ...
            @if($search)
                <a href="{{ route('kota.transactions.index') }}" class="text-sm text-orange-700 hover:underline py-2">Hapus pencarian</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-bold text-gray-800">Daftar Transaksi ({{ $transactions->total() }})</h3>
        </div>
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-gray-50 text-gray-600 text-xs font-semibold uppercase border-b">
                        ...
                        <th class="px-6 py-3">Biaya Antar</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($transactions as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3">Rp{{ number_format($item->subtotal ?? 0, 0, ',', '.') }}</td>
                            <td class="px-6 py-3">Rp{{ number_format($item->delivery_fee ?? 0, 0, ',', '.') }}</td>
                            <td class="px-6 py-3 font-semibold">Rp{{ number_format($item->total_amount ?? 0, 0, ',', '.') }}</td>
                            <td class="px-6 py-3">{{ $item->payment_method_snapshot ?? '-' }}<br><span class="text-xs">{!! statusBadge($item->payment_status ?? '-') !!}</span></td>
                            <td class="px-6 py-3 text-xs text-gray-500">{{ $item->created_at ? \Carbon\Carbon::parse($item->created_at)->format('d M Y H:i') : '-' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="9" class="px-6 py-6 text-center text-gray-500">Belum ada transaksi di area Anda.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
