...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Deliveries — Kirim Barang</h1>
            <p class="text-sm text-gray-500">Monitoring order kirim barang di area coverage Anda</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-xl shadow border border-gray-100 mb-6">
        <form action="{{ route('kota.deliveries.index') }}" method="GET" class="flex flex-wrap gap-3 items-end">
            <div>
            ...
            @if($search)
                <a href="{{ route('kota.deliveries.index') }}" class="text-sm text-orange-700 hover:underline py-2">Hapus pencarian</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-bold text-gray-800">Daftar Deliveries ({{ $deliveries->total() }})</h3>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($deliveries as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-gray-600">{{ ucfirst(strtolower($item->vehicle_type ?? '-')) }}</td>
                            <td class="px-6 py-3">{{ $item->recipient_name ?? '-' }}</td>
                            <td class="px-6 py-3 font-semibold">Rp{{ number_format($item->total_amount ?? 0, 0, ',', '.') }}</td>
                            <td class="px-6 py-3">{!! statusBadge(strtoupper($item->status ?? '')) !!}</td>
                            <td class="px-6 py-3 text-xs text-gray-500">{{ $item->created_at ? \Carbon\Carbon::parse($item->created_at)->format('d M Y H:i') : '-' }}</td>
                            <td class="px-6 py-3">
                                <a href="{{ route('kota.deliveries.show', $item->id) }}" class="text-orange-600 hover:text-orange-800 font-medium text-xs"><i class="fa-solid fa-eye mr-1"></i>Detail</a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="9" class="px-6 py-6 text-center text-gray-500">Belum ada delivery di area Anda.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
