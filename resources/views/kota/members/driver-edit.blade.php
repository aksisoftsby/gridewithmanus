...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="{{ route('kota.members.index', ['type' => 'driver']) }}" class="text-sm text-purple-600 hover:underline">
    ...
    <form method="POST" action="{{ route('kota.members.driver.update', $driver->id) }}" class="bg-white rounded-xl shadow border border-gray-100 p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.1" min="0" max="5" name="rating" value="{{ $driver->rating }}" class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Terverifikasi</label>
                <label class="inline-flex items-center mt-2">
                    <input type="checkbox" name="is_verified" value="1" {{ !empty($driver->is_verified) ? 'checked' : '' }} class="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="any" name="current_lat" value="{{ $driver->current_lat }}" class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Longitude Lokasi Saat Ini</label>
                <input type="number" step="any" name="current_lng" value="{{ $driver->current_lng }}" class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
            </div>
        </div>
