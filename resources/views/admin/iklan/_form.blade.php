...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.iklan.index') }}" class="text-pink-700 hover:underline text-sm font-semibold">&larr; Kembali</a>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow border border-gray-100">
        <form action="{{ isset($iklan) ? route('admin.iklan.update', $iklan->id) : route('admin.iklan.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" required value="{{ old('title', $iklan->title ?? '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600" placeholder="Misal: Rumah dijual cepat di Kediri">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Kategori</label>
                    <select name="category_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
                ...
                <div>
                    ...
                    <input type="number" step="100" name="price" value="{{ old('price', $iklan->price ?? 0) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
                </div>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi</label>
                <textarea name="description" rows="4" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600" placeholder="Deskripsikan iklan secara singkat...">{{ old('description', $iklan->description ?? '') }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Kontak</label>
                    <input type="text" name="contact_name" value="{{ old('contact_name', $iklan->contact_name ?? '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">No. HP</label>
                    <input type="text" name="contact_phone" value="{{ old('contact_phone', $iklan->contact_phone ?? '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Kota</label>
                    <input type="text" name="city" value="{{ old('city', $iklan->city ?? '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600" placeholder="Kediri">
                </div>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Foto Iklan (URL, maksimal 10 foto — satu URL per baris)</label>
                <textarea name="photos" rows="5" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600 font-mono text-xs" placeholder="https://example.com/foto1.jpg&#10;https://example.com/foto2.jpg">{{ old('photos', isset($iklan) ? implode("\n", json_decode($iklan->photos ?? '[]', true) ?? []) : '') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="expired_months" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
                        @for($m = 1; $m <= 12; $m++)
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600">
