...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.iklan.index') }}" class="text-pink-700 hover:underline text-sm font-semibold">&larr; Iklan</a>
    </div>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    @forelse($categories as $cat)
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                @if($cat->is_active)
                                    <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-pink-100 text-pink-900">Aktif</span>
                                @else
                                    ...
                                    <div class="bg-white rounded-xl shadow-lg p-6 w-96">
                                        ...
                                        <form action="{{ route('admin.iklan.kategori.update', $cat->id) }}" method="POST">
                                            ...
                                            <label class="flex items-center space-x-2 mb-4">
                                                <input type="checkbox" name="is_active" value="1" {{ $cat->is_active ? 'checked' : '' }} class="rounded border-gray-300 text-pink-700">
                                                <span class="text-sm">Aktif</span>
                                            </label>
                                            <div class="flex space-x-2">
                                                <button type="submit" class="bg-pink-700 text-white px-4 py-2 rounded-lg text-sm">Simpan</button>
        ...
        <div class="bg-white p-6 rounded-xl shadow border border-gray-100 h-fit">
            ...
            <form action="{{ route('admin.iklan.kategori.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-pink-600" placeholder="Misal: Elektronik">
                </div>
