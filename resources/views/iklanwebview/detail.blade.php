...
@section('content')
<a href="{{ url()->previous() === request()->fullUrl() ? route('iklanwebview.index') : url()->previous() }}" class="inline-flex items-center text-sm text-purple-700 font-medium mb-3">
...
<div class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
    ...
    <div class="p-5">
        @if($item->category_name)
            <span class="inline-block bg-purple-700 text-white text-xs font-semibold px-3 py-1 rounded-full mb-2">{{ $item->category_name }}</span>
        @endif
        <h1 class="text-lg font-bold text-gray-900">{{ $item->title }}</h1>
        @if((float)$item->price > 0)
            <div class="text-xl font-extrabold text-purple-700 mt-1">Rp {{ number_format((float)$item->price, 0, ',', '.') }}</div>
        @endif
        ...
        @if($item->contact_name || $item->contact_phone)
            <div class="mt-4 border-t pt-4 space-y-1 text-sm">
                ...
                @if($item->contact_phone)
                    <p><i class="fa-solid fa-phone mr-2 text-gray-400"></i><a href="tel:{{ $item->contact_phone }}" class="text-purple-700 font-semibold">{{ $item->contact_phone }}</a></p>
                @endif
            </div>
        @endif

        @if($me && $isOwner)
            <div class="mt-4 bg-amber-50 border-l-4 border-amber-500 p-3 rounded text-xs text-amber-700">
